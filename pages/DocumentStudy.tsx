...
import { getDocumentById, updateDocumentSummary, saveFlashcards, getFlashcards, saveQuizAttempt, updateDocumentNotes, saveDocumentChatMessage } from '../services/storageService';
import { generateSummary, generateFlashcards, generateQuiz, createChatSession, generateExplanation, generateAudioLesson, generateStudyNotes, generateImportantPoints } from '../services/geminiService';
import { Document, ChatMessage, Flashcard, QuizQuestion } from '../types';
import { PDFViewer } from '../components/PDFViewer';
import { MessageSquare, FileText, BrainCircuit, PenTool, Send, Loader2, Sparkles, RefreshCw, Bookmark, Check, X, ArrowLeft, ArrowRight, Layers, Lightbulb, Grid, List, Settings, ExternalLink, BookOpen, Mic, Play, Square, FileEdit, Save, Pause, Headphones, Globe, DownloadCloud, Minimize2, Maximize2, Target } from 'lucide-react';

import { ChatSession } from '../services/geminiService';

type Tab = 'pdf' | 'chat' | 'summary' | 'flashcards' | 'quiz' | 'audio' | 'notes' | 'important-points';
...
export const DocumentStudy: React.FC = () => {
  ...
  // Important Points State
  const [importantPoints, setImportantPoints] = useState('');
  const [isImportantPointsLoading, setIsImportantPointsLoading] = useState(false);

  // Error states
  const [flashcardError, setFlashcardError] = useState('');
  const [quizError, setQuizError] = useState('');

  const getAudioCacheStorageKey = (docId: string) => `learnai_audio_cache_${docId}`;
  const getScriptCacheStorageKey = (docId: string) => `learnai_script_cache_${docId}`;

  const loadLocalAudioCache = (docId: string) => {
    try {
      const rawAudio = localStorage.getItem(getAudioCacheStorageKey(docId));
      const rawScript = localStorage.getItem(getScriptCacheStorageKey(docId));
      const audio = rawAudio ? JSON.parse(rawAudio) : {};
      const script = rawScript ? JSON.parse(rawScript) : {};
      return { audio, script };
    } catch {
      return { audio: {}, script: {} };
    }
  ...
  const saveLocalAudioCache = (docId: string, audio: Record<string, string>, script: Record<string, string>) => {
    localStorage.setItem(getAudioCacheStorageKey(docId), JSON.stringify(audio));
    localStorage.setItem(getScriptCacheStorageKey(docId), JSON.stringify(script));
  };
  useEffect(() => {
    const loadDocument = async () => {
      ...
      if (foundDoc) {
        ...
        if (foundDoc.importantPoints) setImportantPoints(foundDoc.importantPoints);
        if (foundDoc.quiz?.questions?.length) setQuizQuestions(foundDoc.quiz.questions);
        ...
        // Restore Cloudinary audio URLs and script fallbacks from MongoDB
        let urlCache: Record<string, string> = {};
        let sCache: Record<string, string> = {};

        if (foundDoc.audioCache && foundDoc.audioCache.length > 0) {
          foundDoc.audioCache.forEach((entry) => {
            const rawKey = (entry.key || '').trim();
            if (!rawKey) return;

            const keysToStore = new Set<string>([rawKey]);
            if (rawKey.endsWith('_english')) keysToStore.add(rawKey.replace(/_english$/, '_English'));
            if (rawKey.endsWith('_hinglish')) keysToStore.add(rawKey.replace(/_hinglish$/, '_Hinglish'));

            keysToStore.forEach((k) => {
              if (entry.url) urlCache[k] = entry.url;
              if (entry.scriptText) sCache[k] = entry.scriptText;
            });
          });
        }

        // Merge local cache so refresh still shows Play even if backend payload lags
        const localCache = loadLocalAudioCache(foundDoc.id);
        urlCache = { ...urlCache, ...localCache.audio };
        sCache = { ...sCache, ...localCache.script };

        setAudioCache(urlCache);
        setScriptCache(sCache);

        const session = createChatSession(foundDoc.text);
        setChatSession(session);

        const restoredHistory = (foundDoc.chatHistory || [])
          .map((m: any, index: number) => ({
            id: m.id || `hist-${index}-${Date.now()}`,
            role: m.role === 'assistant' ? 'model' : 'user',
            text: m.text || m.content || '',
            timestamp: typeof m.timestamp === 'number' ? m.timestamp : new Date(m.timestamp || Date.now()).getTime()
          }))
          .filter((m: any) => Boolean(m.text));

        if (restoredHistory.length > 0) {
          setChatMessages(restoredHistory as ChatMessage[]);
        } else {
          setChatMessages([{
            id: 'init',
            role: 'model',
  ...
  const handleSendMessage = async () => {
    if (!inputMessage.trim() || !doc?.text) return;

    const session = chatSession ?? createChatSession(doc.text);
    ...
    if (doc?.id) {
      void saveDocumentChatMessage(doc.id, 'user', userMsg.text || '');
    }
    setInputMessage('');
    ...
    try {
      ...
      if (doc?.id) {
        void saveDocumentChatMessage(doc.id, 'assistant', modelMsg.text || '');
      }
    } catch (err) {
      console.error(err);
      ...
      if (doc?.id) {
        void saveDocumentChatMessage(doc.id, 'assistant', 'Sorry, I encountered an error communicating with Gemini.');
      }
    } finally {
  ...
  const handleGenerateSummary = async (force = false) => {
    ...
    const text = await generateSummary(doc.text, doc.id, force);
    setSummary(text);
  ...
  const handleGenerateFlashcards = async (force = false) => {
    ...
    setFlashcardError('');
    const newCards = await generateFlashcards(doc.text, doc.id, force);
  ...
  const handleGenerateQuiz = async (force = false) => {
    ...
    setQuizError('');
    const questions = await generateQuiz(doc.text, questionCount, doc.id, force);
  ...
  const handleGenerateAudio = async (identifier: number | 'summary', content: string) => {
    ...
    if (audioCache[key] || scriptCache[key]) return; // already cached

    setGeneratingKey(key);
    const type = identifier === 'summary' ? 'summary' : 'detail';
    const pageKey = identifier === 'summary' ? 'summary' : String(identifier);
    const result = await generateAudioLesson(content, doc?.id, audioLanguage, type, pageKey);

    if (result.audioUrl) {
      setAudioCache(prev => {
        const next = { ...prev, [key]: result.audioUrl! };
        if (doc?.id) saveLocalAudioCache(doc.id, next, scriptCache);
        return next;
      });
    } else if (result.scriptText) {
      setScriptCache(prev => {
        const next = { ...prev, [key]: result.scriptText! };
        if (doc?.id) saveLocalAudioCache(doc.id, audioCache, next);
        return next;
      });
    }
  ...
  const handleGenerateAll = async () => {
    ...
    if (!audioCache[`summary_${audioLanguage}`] && !scriptCache[`summary_${audioLanguage}`]) {
      await handleGenerateAudio('summary', doc.text);
    }
    for (const page of audioPages) {
      if (!audioCache[`${page.number}_${audioLanguage}`] && !scriptCache[`${page.number}_${audioLanguage}`]) {
  ...
  const handlePlayAudio = async (identifier: number | 'summary') => {
    ...
    const scriptText = scriptCache[key];
    ...
    try {
      ...
      if (audioUrl) {
        ...
        const isSameAudio = activeAudioKey === key && audio.src === audioUrl;

        // Resume from paused position for the same audio instead of restarting
        if (isSameAudio && audio.paused && audio.currentTime > 0) {
          await audio.play();
          setIsPlaying(true);
          return;
        }

        // Switching audio or first play should reset previous playback
        ...
        audio.onerror = () => {
          ...
          setIsPlaying(false);
          alert('Error playing audio. Please regenerate.');
        };
        await audio.play();
      } else if (scriptText) {
        const isSameScript = activeAudioKey === key;

        // Resume paused speech for same script
        if (isSameScript && window.speechSynthesis.paused) {
        ...
        utterance.lang = audioLanguage === 'Hinglish' ? 'hi-IN' : 'en-US';
        utterance.rate = 0.95;
  ...
  const handlePauseAudio = async () => {
    ...
    } else if (audioContextRef.current && audioSourceRef.current) {
  ...
  const handleGenerateNotes = async (force = false) => {
    ...
    const generatedNotes = await generateStudyNotes(doc.text, doc.id, force);
  ...
  const handleGenerateImportantPoints = async (force = false) => {
    if (!doc?.text) return;
    
    setIsImportantPointsLoading(true);
    try {
      const points = await generateImportantPoints(doc.text, doc.id, force);
      setImportantPoints(points);
    } catch (error) {
      console.error('Failed to generate important points:', error);
      setImportantPoints('<p class="text-red-600">Failed to generate important points. Please try again.</p>');
    } finally {
      setIsImportantPointsLoading(false);
    }
  };
  ...
  const tabs: { id: Tab; label: string; icon: React.ElementType }[] = [
    ...
    { id: 'important-points', label: 'Key Points', icon: Target },
             ...
             <div className="bg-indigo-50 dark:bg-gray-900 border-b dark:border-gray-700 px-4 py-2 flex items-center justify-between shadow-sm z-10 transition-all">
                <div className="flex items-center gap-3 overflow-hidden">
                   ...
                   <div className="flex flex-col">
                      ...
                      {(audioCache[`${pdfCurrentPage}_${audioLanguage}`] || scriptCache[`${pdfCurrentPage}_${audioLanguage}`]) ? (
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {chatMessages.map((msg) => (
                <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] p-3 rounded-lg text-sm shadow-sm whitespace-pre-wrap leading-relaxed ${
                    msg.role === 'user' 
            ...
            {summary ? (
              <div className="w-full">
                <div className="flex justify-between items-center mb-6 pb-4 border-b dark:border-gray-700">
                    ...
                    <button onClick={() => handleGenerateSummary(true)} className="text-sm flex items-center gap-1 text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 font-medium px-3 py-1 bg-indigo-50 dark:bg-indigo-900/30 rounded-md">
                 ...
                 <div className="shrink-0">
                    {generatingKey === `summary_${audioLanguage}` ? (
                      ...
                      activeAudioKey === `summary_${audioLanguage}` && isPlaying ? (
                        ...
                        (audioCache[`summary_${audioLanguage}`] || scriptCache[`summary_${audioLanguage}`]) ? (
                            <button 
                     ...
                     <div className="shrink-0">
                        {generatingKey === `${page.number}_${audioLanguage}` ? (
                          ...
                          activeAudioKey === `${page.number}_${audioLanguage}` && isPlaying ? (
                            ...
                            (audioCache[`${page.number}_${audioLanguage}`] || scriptCache[`${page.number}_${audioLanguage}`]) ? (
                                <button 
            ...
            {importantPoints ? (
              <div className="w-full">
                <div className="flex justify-between items-center mb-6 pb-4 border-b dark:border-gray-700">
                  ...
                  <button
                    onClick={() => handleGenerateImportantPoints(true)}
                    disabled={isImportantPointsLoading}
                    className="text-sm flex items-center gap-1 text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 font-medium px-3 py-1 bg-indigo-50 dark:bg-indigo-900/30 rounded-md"
                  >
                    <RefreshCw className="w-3 h-3" /> Regenerate
                  </button>
                </div>
                <div
                  className="bg-white dark:bg-gray-800 p-8 rounded-xl border dark:border-gray-700 shadow-sm text-gray-900 dark:text-gray-100"
                  dangerouslySetInnerHTML={{ __html: importantPoints }}
                />
              </div>
            ) : (
              <div className="flex flex-col items-center justify-center h-full text-center">
                {isImportantPointsLoading ? (
                  <>
                    <Loader2 className="animate-spin w-12 h-12 text-indigo-600 mb-4" />
                    <p className="text-gray-600 dark:text-gray-400">Extracting important points from your document...</p>
                  </>
                ) : (
                  <>
                    <Target className="w-16 h-16 text-indigo-100 dark:text-indigo-900/50 mb-6" />
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">No Key Points Yet</h3>
                    <p className="text-gray-500 dark:text-gray-400 mb-6 max-w-sm">Extract the most important concepts, facts, and exam-likely points from your document using AI.</p>
                    <button
                      onClick={handleGenerateImportantPoints}
                      disabled={isImportantPointsLoading}
                      className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-xl flex items-center gap-2 disabled:opacity-50 shadow-md transition-all hover:scale-105"
                    >
        ...
        {activeTab === 'flashcards' && (
          <div className="flex flex-col h-full max-w-6xl mx-auto w-full">
            <div className="p-4 border-b dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-900">
              ...
              <button onClick={() => handleGenerateFlashcards(true)} className="text-sm flex items-center gap-1 text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 font-medium bg-white dark:bg-gray-800 border dark:border-gray-600 px-3 py-1.5 rounded-md shadow-sm">
                  <RefreshCw className="w-3 h-3" /> Regenerate
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-gray-50/50 dark:bg-gray-950">
              {cards.length > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center h-full text-center">
                    ...
                    {flashcardError && <p className="text-red-500 text-sm mb-4">{flashcardError}</p>}
                    <button
                  ...
                  <div className="w-full space-y-8">
                      ...
                      <div className="space-y-6">
                        {quizQuestions.map((q, i) => (
                          <div key={q.id ?? i} className="border border-gray-200 dark:border-gray-700 rounded-xl p-6 bg-white dark:bg-gray-800 shadow-sm">
