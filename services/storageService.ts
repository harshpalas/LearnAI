...
export const getDocuments = async (): Promise<Document[]> => {
  ...
  if (token) {
    try {
      ...
      const docsWithIds = Array.isArray(docs) ? docs.map(doc => {
        if (doc._id && !doc.id) doc.id = doc._id;
        return doc;
      }) : [];
      return docsWithIds.sort((a, b) => new Date(b.uploadDate).getTime() - new Date(a.uploadDate).getTime());
    } catch (error) {
...
export const getDocumentById = async (id: string): Promise<Document | undefined> => {
  // Guard clause: Check if ID is valid before making API call
  if (!id || id === 'undefined' || id.trim() === '') {
    console.warn('getDocumentById called with invalid ID:', id);
    return undefined;
  }
  ...
  // Fallback to local storage
  return get<Document>(KEYS.DOCS).find(d => d.id === id || (d as any)._id === id);
};
...
export const updateDocumentImportantPoints = async (id: string, importantPoints: string) => {
  const token = getAuthToken();
  if (token) {
    try {
      await apiClient.put(`/api/documents/${id}`, { importantPoints });
      return;
    } catch (error) {
      console.error('Failed to update importantPoints on backend, using local storage:', error);
    }
  }
  ...
  const docIndex = docs.findIndex(d => d.id === id);
  if (docIndex > -1) {
    docs[docIndex].importantPoints = importantPoints;
    set(KEYS.DOCS, docs);
  }
};


export const saveDocumentChatMessage = async (id: string, role: 'user' | 'assistant', content: string) => {
  if (!id || !content?.trim()) return;

  const token = getAuthToken();
  if (token) {
    try {
      await apiClient.patch(`/api/documents/${id}/chat`, { role, content });
      return;
    } catch (error) {
      console.error('Failed to save chat message on backend, using local storage fallback:', error);
    }
  }

  const docs = get<Document>(KEYS.DOCS);
  const docIndex = docs.findIndex(d => d.id === id || (d as any)._id === id);
  if (docIndex > -1) {
    if (!docs[docIndex].chatHistory) docs[docIndex].chatHistory = [];
    docs[docIndex].chatHistory!.push({
      role,
      content,
      text: content,
      timestamp: Date.now()
